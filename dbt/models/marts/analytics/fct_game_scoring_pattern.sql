...
with game_scoring as (
    select
        ...
        start_time,
        'HOME' as team_type,
        coalesce(home_name, '') as team_name,
        -- Quarter Scoring
    ...
    select
        ...
        start_time,
        'AWAY' as team_type,
        coalesce(away_name, '') as team_name,
    ...
    select
        ...
        home_ot_scores as ot_scores,
        home_q1_score + home_q2_score as first_half_score,
        coalesce(home_q3_score, 0) + coalesce(home_q4_score, 0) as second_half_score,
        home_score as total_score,
        home_q1_score / nullif(home_score, 0) * 100 as q1_scoring_pct,
        home_q2_score / nullif(home_score, 0) * 100 as q2_scoring_pct,
        home_q3_score / nullif(home_score, 0) * 100 as q3_scoring_pct,
    ...
    select
        ...
        away_ot_scores as ot_scores,
        away_q1_score + away_q2_score as first_half_score,
        coalesce(away_q3_score, 0) + coalesce(away_q4_score, 0) as second_half_score,
        away_score as total_score,
        away_q1_score / nullif(away_score, 0) * 100 as q1_scoring_pct,
        away_q2_score / nullif(away_score, 0) * 100 as q2_scoring_pct,
        away_q3_score / nullif(away_score, 0) * 100 as q3_scoring_pct,
    ...
    select
        ...
        'HOME' as team_type,
        coalesce(home_name, '') as team_name,
        home_p1_score as q1_score,
        home_p2_score as q2_score,
        home_p3_score as q3_score,
        null as q4_score,
        home_ot_scores as ot_scores,
        home_p1_score + (home_p2_score/2)  as first_half_score,
        (home_p2_score/2) + coalesce(home_p3_score, 0) as second_half_score,
        home_score as total_score,
        home_p1_score / nullif(home_score, 0) * 100 as q1_scoring_pct,
        home_p2_score / nullif(home_score, 0) * 100 as q2_scoring_pct,
        home_p3_score / nullif(home_score, 0) * 100 as q3_scoring_pct,
        null q4_scoring_pct,
        case when home_score > away_score then true else false end as is_winner,
        abs(home_score - away_score) as score_margin,
        case
        ...
        case
            when away_p1_score > away_p2_score and away_p1_score > away_p3_score then 'Q1'
            when away_p2_score > away_p1_score and away_p2_score > away_p3_score then 'Q2'
            else 'Q3'
        end as highest_scoring_quarter,
        number_of_overtimes,
        partition_year,
    ...
    select
        ...
        'AWAY' as team_type,
        coalesce(away_name, '') as team_name,
        away_p1_score as q1_score,
        away_p2_score as q2_score,
        away_p3_score as q3_score,
        null as q4_score,
        away_ot_scores as ot_scores,
        away_p1_score + (away_p2_score/2) as first_half_score,
        (away_p2_score/2) + coalesce(away_p3_score, 0) as second_half_score,
        away_score as total_score,
        away_p1_score / nullif(away_score, 0) * 100 as q1_scoring_pct,
        away_p2_score / nullif(away_score, 0) * 100 as q2_scoring_pct,
        away_p3_score / nullif(away_score, 0) * 100 as q3_scoring_pct,
