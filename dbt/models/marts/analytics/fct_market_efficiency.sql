...
with market_analysis as (
    select
        ...
        round(avg(market_vig), 3) as avg_vig,
        min(market_vig) as min_vig,
        max(market_vig) as max_vig,
        -- Market Classification
        case
            when avg(market_vig) <= 5 then 'Highly Efficient'
            when avg(market_vig) <= 10 then 'Efficient'
            when avg(market_vig) <= 15 then 'Moderately Efficient'
            else 'Inefficient'
        end as market_efficiency_rating,

        -- Probability Analysis
        round(avg(probability_delta), 3) as avg_probability_spread,
